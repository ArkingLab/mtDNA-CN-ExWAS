...
dir2=/dcl01/arking/data/static/UKBiobank/WES/plink/200k.merged.common_rare_genotypes/lmm/
...
bolt \
	...
	--phenoFile=${dir2}/200k_wes.lmm.pheno.txt \
	--phenoCol=mtDNA_CN \
	--reml \
    ...
    --covarFile=${dir2}/200k_wes.lmm.covar.txt \
    --covarCol=wes.batch \
	--LDscoresFile=${bolt}/tables/LDSCORE.1000G_EUR.tab.gz \
	--geneticMapFile=${bolt}/tables//genetic_map_hg19_withX.txt.gz \
	--statsFile=${dir2}/chr1_to_chr22.stats.gz \
    --numThreads=8 \
	--verboseStats \
	2>&1 | tee reml.log # log output written to stdout and stderr
