#!/usr/bin/env Rscript
# Converts UK Biobank WES plink binary fileset for a single chromosome --> GDS
# Uses `SeqArray::seqBED2GDS`
...
p <- arg_parser("UKBiobank WES plink binary fileset to GDS for a single chromosome.")
p <- add_argument(p, "chr", help="Chromosome, values: 1-22,X,Y [or 23,24]")
p <- add_argument(p, "--bed_dir", 
                  default='/dcl01/arking/data/static/UKBiobank/WES/plink/', 
                  help='Top level of PLINK binary fileset directory.')
p <- add_argument(p, "--output_dir",
                  default="/dcs04/arking/data/active/projects/mito_rare-variant/burden/genesis/bed2gds/",
...
chr <- argv$chr
file_prefix <- file.path(argv$bed_dir, paste0("UKBexomeOQFE_chr", chr))
bed_file <- paste0(file_prefix, ".bed")
bim_file <- paste0(file_prefix, ".bim")
fam_file <- paste0(file_prefix, ".fam")
output_file <- file.path(argv$output_dir, paste0("UKBexomeOQFE_chr", chr, ".gds"))
